...
RegisterNetEvent('shady-vehicleshop:server:addPlayer', function(citizenid)
    financetimer[citizenid] = os.time()
end)

-- Deduct stored game time from player on logout
RegisterNetEvent('shady-vehicleshop:server:removePlayer', function(citizenid)
...
-- Callbacks
QBCore.Functions.CreateCallback('shady-vehicleshop:server:getVehicles', function(source, cb)
    local src = source
...
RegisterNetEvent('shady-vehicleshop:server:deleteVehicle', function(netId)
    local vehicle = NetworkGetEntityFromNetworkId(netId)
    DeleteEntity(vehicle)
end)

-- Sync vehicle for other players
RegisterNetEvent('shady-vehicleshop:server:swapVehicle', function(data)
    local src = source
    TriggerClientEvent('shady-vehicleshop:client:swapVehicle', -1, data)
    Wait(1500)                                                -- let new car spawn
    TriggerClientEvent('shady-vehicleshop:client:homeMenu', src) -- reopen main menu
end)

-- Send customer for test drive
RegisterNetEvent('shady-vehicleshop:server:customTestDrive', function(vehicle, playerid)
    local src = source
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target))) < 3 then
        TriggerClientEvent('shady-vehicleshop:client:customTestDrive', target, vehicle)
    else
...
RegisterNetEvent('shady-vehicleshop:server:financePayment', function(paymentAmount, vehData)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('shady-vehicleshop:server:financePaymentFull', function(data)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local cash = player.PlayerData.money['cash']
    local bank = player.PlayerData.money['bank']
...
RegisterNetEvent('shady-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    local src = source
    ...
    if cash > tonumber(vehiclePrice) then
        ...
        TriggerClientEvent('shady-vehicleshop:client:buyShowroomVehicle', src, vehicle, plate)
...
RegisterNetEvent('shady-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    local src = source
...
RegisterNetEvent('shady-vehicleshop:server:sellShowroomVehicle', function(data, playerid)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target.PlayerData.source))) < 3 then
        ...
        if cash >= tonumber(vehiclePrice) then
            ...
            TriggerClientEvent('shady-vehicleshop:client:buyShowroomVehicle', target.PlayerData.source, vehicle, plate)
            ...
            exports['ps-banking']:AddMoney(player.PlayerData.job.name, vehiclePrice, 'Vehicle sale')
...
RegisterNetEvent('shady-vehicleshop:server:sellfinanceVehicle', function(downPayment, paymentAmount, vehicle, playerid)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local target = QBCore.Functions.GetPlayer(tonumber(playerid))
...
RegisterNetEvent('shady-vehicleshop:server:checkFinance', function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
